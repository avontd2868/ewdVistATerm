var VistATerm = function(params) {
  ...
  io.sockets.on('connection', function(client){
    ...
    var m = spawn(params.spawnVistA.command, params.spawnVistA.arguments);
    m.isRunning = true;
    ...
    m.on("exit",function(code,signal) {
      if (params.log) console.log(client.sessionId + ": back-end routine has exited");
      m.isRunning = false;
...
term.start({
  ...
  webServer: {
    ...
    ssl: true,
    options: {
      key: '/opt/node/ssl/myserver.key',
      cert: '/opt/node/ssl/voesolutions.crt'
    }
  },
  addLF: false,
  echo: true
